...
function randMatch() {
  const randMatchUrl =
    ServerConfig.urlPrefix + ServerConfig.apiMap['team']['match'];
  axios
    .get(randMatchUrl, {
      params: {
        route: route.value,
  ...
  >
    <n-collapse-item title="🏊 &nbsp;&nbsp;通过团队密码加入团队" name="1">
      <n-space style="margin-top: 10px" :size="30" :vertical="true">
        <n-input
          ...
          type="text"
          placeholder="输入你的团队编号"
        />
        <n-input
          v-model:value="teamPass"
          type="text"
          placeholder="输入你的团队密码"
        />
        <n-button @click="joinTeam" style="width: 100%" type="primary"
          >加入团队</n-button
        >
    ...
    <n-collapse-item title="🙈 &nbsp;&nbsp;随机组队" name="2">
      <n-space style="margin-top: 10px" :size="30" :vertical="true">
        ...
        ></n-select>
        <n-button @click="randMatch" style="width: 100%" type="primary"
          >随机匹配</n-button
        >
        <n-button @click="goBack" style="width: 100%">返回上一步</n-button>
      </n-space>
